...
def _is_source_file(rel_path: str) -> bool:
    ...
    return rel_path.endswith(SOURCE_EXT) and any(
...
def _classify_role(rel_path: str, public_functions: list[dict]) -> str:
    ...
    if rel_path.startswith("core/"):
        ...
        if "system_model" in lowered:
            return "architecture_model"
    if any(f["name"] == "main" for f in public_functions):
        return "entrypoint"
    return "module"
...
def update_system_model(
    ...
    tracked_changes = {_rel_path(path, repo_root) for path in changed_files if _is_source_file(path)}
    if not modules:
    ...
    model["stats"] = {
        "module_count": len(modules),
        "public_api_count": sum(data.get("public_api_count", 0) for data in modules.values()),
        "dependency_edges": sum(len(data.get("dependencies", [])) for data in modules.values()),
